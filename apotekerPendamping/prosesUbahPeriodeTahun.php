...
if (isset($_POST['submit'])) {
    ...
    $id_periode_tahun = $_POST['id_periode_tahun'];
    $tahun = $_POST['tahun'];

    // Sanitasi input
    $id_periode_tahun = mysqli_real_escape_string($conn, $id_periode_tahun);
    $bulan = mysqli_real_escape_string($conn, $tahun);

    // Cek apakah ID pengguna ada di database
    $result = mysqli_query($conn, "SELECT * FROM periode_tahun WHERE id_periode_tahun='$id_periode_tahun'");
    if (mysqli_num_rows($result) == 0) {
        die("ID periode tidak ditemukan.");
    }

    // Tambahkan kondisi pengecekan jika ada data dengan bulan dan tahun yang sama
    $checkQuery = "SELECT * FROM periode_tahun WHERE tahun='$tahun' AND id_periode_tahun != '$id_periode_tahun'";
    $checkResult = mysqli_query($conn, $checkQuery);
    if (mysqli_num_rows($checkResult) > 0) {
        echo "<script>alert('Data dengan bulan yang sama sudah ada!');</script>";
        echo "<script>window.location.href = 'periodeTahun.php';</script>";
        exit();
    }

    // Hapus kondisi pengecekan bulan dan tahun jika kosong
    $query = "UPDATE periode_tahun SET tahun='$tahun' WHERE id_periode_tahun='$id_periode_tahun'";
